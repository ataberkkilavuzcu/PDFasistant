    ...
    <div className="relative w-full" ref={dropdownRef}>
      <form onSubmit={handleSubmit} className="flex gap-2">
        <div className="relative flex-1 group">
          <div className="absolute inset-0 bg-primary-500/20 rounded-lg blur opacity-0 group-focus-within:opacity-100 transition-opacity duration-300"></div>
          <input
            type="text"
            value={query}
        ...
        <button
          ...
          className="px-6 py-2 bg-primary-600/80 hover:bg-primary-500 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-primary-500/20 border border-white/5 font-medium backdrop-blur-sm"
        >
          {isSearching ? (
      ...
      {isOpen && results.length > 0 && (
        <div className="absolute z-50 w-full mt-3 bg-[#1a1a1a]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl max-h-80 overflow-y-auto custom-scrollbar animate-fade-in">
            ...
            {results.map((result, index) => (
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-sm font-medium text-primary-400 group-hover:text-primary-300">
