  ...
  useEffect(() => {
    ...
    const initPDF = async () => {
      try {
        ...
        // Race between initialization and a safety timeout
        await Promise.race([
          initializePDFJS(),
          new Promise<void>((_, reject) =>
            setTimeout(() => reject(new Error('PDF.js init timeout')), 8000)
          ),
        ]);
        if (mounted) {
        ...
        if (mounted) {
          ...
          // Always mark initialization as done so the uploader becomes usable
          // PDF.js will try to initialize again when a file is actually selected
          setIsInitializing(false);
        }
      }
    };
  ...
  return (
    <main className="min-h-screen relative overflow-hidden bg-[#0a0a0b] text-white selection:bg-emerald-500/30">
          ...
          {recentDocuments.length > 0 && !isProcessing && (
            <div className="mt-8 w-full animate-fade-in">
              <DocumentList
                ...
                onDocumentRenamed={() => loadRecentDocuments()}
              />
            </div>
          )}
        </div>
      ...
      <footer className="mt-24 mb-8 text-center">
        <div className="flex items-center justify-center gap-4 text-xs text-gray-500" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
          <span>Built with Next.js, Tailwind CSS &amp; Gemini AI</span>
